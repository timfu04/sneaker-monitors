...
def get_product_info_from_raffle_page(session):
  ...
  products = res.html.find("section#products-section",
                           first=True).find("article")
  ...
  scraped_first_product_raffle_url_dict = dict(
    map(lambda key: (key, scraped_first_product_dict[key]), ["raffle_url"]))
...
async def get_product_info_from_pdp(a_session, url):
  ...
  product_desc = res.html.find("div#main-product-description",
                               first=True).find("li")
...
async def main(product_url_list):
  ...
  tasks = (get_product_info_from_pdp(a_session, url)
...
if __name__ == "__main__":
  ...
  os.chdir(fr'{config["atmos_replit"]["path"]}/atmos')
