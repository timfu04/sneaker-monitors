...
def run_discord_bot():
  ...
  async def scrape_data():
    ...
    scraped_product_url_list = list(map(lambda x: x.raffle_url, scraped_product_list)) # Get product URL from scraped product list
    scraped_product_desc_draw_info_list = await start_async_pdp_scrape(scraped_product_url_list)
    ...
    if json_product_list == None:
      ...
      if find_missing_products(scraped_product_list, json_product_list) != []: # List not empty means not same
        write_json("data.json", scraped_product_list)
        missing_product_list = find_missing_products(scraped_product_list, json_product_list)
        await send_embed(missing_product_list, malaysia_datetime)
      

  async def send_embed(missing_product_list: List[Product], malaysia_datetime: datetime):
